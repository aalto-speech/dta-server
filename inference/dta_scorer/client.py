...
1. SCALE. These are CEFR values on a 0-6 scale (0=<A1, 1=A1, 2=A2, 3=B1, ...), not a rating
   out of 5. dta-server currently declares `Score = Field(ge=0, le=5)` and CHECKs
   `BETWEEN 0 AND 5`; the values happen to fit, so nothing raises -- but rendering "2.1/5" to
   a learner for what is actually A2 is wrong. Use `cefr_label` / `cefr_label_fine` in the UI.

...
| `CLIENT_API_KEY`       | empty             | Shared key the mobile app sends as `X-Client-Key`. Empty = header ignored. Must differ from `ADMIN_API_KEY` — the app refuses to start otherwise. |
